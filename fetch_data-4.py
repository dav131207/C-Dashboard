...
def get_ads_with_insights() -> list[dict]:
    ...
    params = {
        "fields": "id,name,effective_status,created_time,"
                  "creative{id,effective_instagram_media_id,instagram_permalink_url,object_story_id,instagram_actor_id}",
        "limit": 100, "access_token": ACCESS_TOKEN,
    }
    while url:
        data = get_json(url, params)
    ...
    # Filter: nur Ads, die zum aktuellen IG-Profil gehören.
    # Wir prüfen mehrere Felder, weil Meta je nach Ad-Typ unterschiedlich befüllt:
    #  - creative.instagram_actor_id (klassischer Boost)
    #  - creative.instagram_user_id (manche Reels-Ads)
    #  - effective_instagram_media_id existiert UND zeigt auf eine Media, die uns gehört
    own_media_ids = set()  # gefüllt später beim Index-Bau

    def ad_belongs_to_profile(ad):
        creative = ad.get("creative") or {}
        # 1) direkt über instagram_actor_id
        if creative.get("instagram_actor_id") == IG_USER_ID:
            return True
        # 2) alternativ instagram_user_id
        if creative.get("instagram_user_id") == IG_USER_ID:
            return True
        # 3) Permalink enthält den Username
        permalink = creative.get("instagram_permalink_url") or ""
        if permalink and f"/{IG_USER_ID}/" in permalink:
            return True
        return False

    own_ads = [a for a in ads if ad_belongs_to_profile(a)]
    print(f"   Davon dem aktuellen Profil (via Filter) zugeordnet: {len(own_ads)}")

    # Debug: zeig, was creative-fields enthält, falls Filter 0 ergibt
    if not own_ads and ads:
        sample = ads[0].get("creative") or {}
        print(f"   [Debug] Creative-Felder vom ersten Ad: {list(sample.keys())}")
        ...
        print(f"   [Debug] Verwende Fallback: alle Ads mit gültigem effective_instagram_media_id behalten")
        # Fallback: alle Ads behalten — der Media-ID-Join filtert dann sowieso
        own_ads = [a for a in ads if (a.get("creative") or {}).get("effective_instagram_media_id")]
        print(f"   {len(own_ads)} Ads mit effective_instagram_media_id (Fallback)")
