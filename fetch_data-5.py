...
def get_ads_with_insights() -> list[dict]:
    ...
    params = {
        "fields": "id,name,effective_status,created_time,creative{id,object_story_id}",
        "limit": 100, "access_token": ACCESS_TOKEN,
    }
    while url:
        data = get_json(url, params)
        ...
        url = data.get("paging", {}).get("next", "") if data.get("paging") else ""
        params = None
    print(f"   {len(ads)} Ads insgesamt im Ad-Account.")

    if not ads:
        return []

    # Step 2: Pro Ad das Creative explizit mit ALLEN relevanten Feldern abfragen.
    # Die /act_x/ads Liste gibt manchmal nur reduzierte Creative-Felder zurück;
    # ein direkter Call auf /{creative_id} liefert die vollständige Struktur.
    print(f"   Lade detaillierte Creative-Daten + Insights pro Ad...")
    for i, ad in enumerate(ads, 1):
        ...
        if creative_id:
            creative_full = get_json(f"{BASE_URL}/{creative_id}", {
                "fields": "effective_instagram_media_id,instagram_permalink_url,"
                          "instagram_actor_id,instagram_user_id,object_story_id,"
                          "object_story_spec",
                "access_token": ACCESS_TOKEN,
            })
            # Merge into existing creative dict
            if creative_full:
                ad["creative"] = {**(ad.get("creative") or {}), **creative_full}
    ...
    # Step 3: Filter — Ads, die zum aktuellen IG-Profil gehören
    def extract_ig_media_id(ad):
        """Try every known field to find the linked IG media ID."""
        creative = ad.get("creative") or {}
        # Direct fields
        for key in ("effective_instagram_media_id",):
            if creative.get(key):
                return creative[key]
        # From permalink URL (https://www.instagram.com/p/SHORTCODE/ -- not directly an ID, skip)
        # From object_story_spec (newer Marketing API format)
        oss = creative.get("object_story_spec") or {}
        for sub in ("photo_data", "video_data", "link_data"):
            if sub in oss and isinstance(oss[sub], dict):
                pass  # rarely has IG IDs
        return None

    def ad_belongs_to_profile(ad):
        creative = ad.get("creative") or {}
        if creative.get("instagram_actor_id") == IG_USER_ID:
            return True
        if creative.get("instagram_user_id") == IG_USER_ID:
            return True
        if extract_ig_media_id(ad):
            return True  # let the media-id join filter further
        return False

    own_ads = [a for a in ads if ad_belongs_to_profile(a)]
    print(f"   Davon dem aktuellen Profil zugeordnet: {len(own_ads)}")

    if not own_ads and ads:
        sample = ads[0].get("creative") or {}
        print(f"   [Debug] Felder vom ersten Ad-Creative nach Detail-Query: {list(sample.keys())}")
        print(f"   [Debug] instagram_actor_id: {sample.get('instagram_actor_id')!r}")
        print(f"   [Debug] instagram_user_id:  {sample.get('instagram_user_id')!r}")
        print(f"   [Debug] effective_instagram_media_id: {sample.get('effective_instagram_media_id')!r}")
        print(f"   [Debug] instagram_permalink_url: {sample.get('instagram_permalink_url')!r}")
        print(f"   [Debug] object_story_id: {sample.get('object_story_id')!r}")
        print(f"   [Debug] erwartete IG_USER_ID: {IG_USER_ID!r}")
